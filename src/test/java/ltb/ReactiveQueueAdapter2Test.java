...
public class ReactiveQueueAdapter2Test {

    @Test
    public void testSimple() {
        BlockingQueue<Integer> q = new LinkedBlockingDeque<>();
        Flux<Integer> f = ReactiveQueueAdapter2.of(q);
    ...
    // We left off with the RQA2 experiment here.
    @Test(enabled=false)
    public void testAsyncConsumerLimitRate() {
        final int N = 10000;
        BlockingQueue<Integer> q = new LinkedBlockingDeque<>();
        ReactiveQueueAdapter2<Integer> rqa = new ReactiveQueueAdapter2<>(q);
        Flux<Integer> f = Flux.create(rqa);
        Semaphore semaphore = new Semaphore(0);
        Disposable subscription = f.subscribeOn(Schedulers.parallel(), false).limitRate(100).take(N).collectList().subscribe(
                l -> {
    ...
    public void testAsyncConsumerEmpty() {
        ...
        // wait 5 sec before adding items to the queue. ReactiveQueueAdapter2 uses internal interval of 1 sec
